...
export default function Post({
  ...
  const handlDeleteComment = async (commentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/v1/posts/${commentId}/comment`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <div className="w-full relative max-w-2xl bg-white  rounded-2xl shadow-sm p-6  relative shadow-xl ">
        ...
        <div
          className={` px-4 py-8  origin-left  gap-5 flex flex-col   ml-2 shadow-xl bg-white absolute right-0 top-0 h-full left-full rounded-2xl  transition-all duration-250 ease-in ${open ? "w-75 opacity-100 scale-x-100" : " opacity-0 w-0 px-0 py-0 overflow-hidden scale-x-0 pointer-events-none"} `}
        >
          <div
            ...
            <div className="flex gap-3 items-center ">
              <div>
                <input
                  ...
                  name=""
                  id=""
                  className="p-2 px-7 rounded-2xl bg-gray-100   outline-none"
              ...
              <div className="">
                <button
                  type="submit"
                  className="disabled:cursor-not-allowed p-1  cursor-pointer    rounded-full"
                  disabled={!mycomment}
                >
          ...
          <div className="flex flex-col gap-3 flex-1 overflow-y-auto relative  ">
            {Loading ? (
              <div className="absolute top-50 right-27">
                <PulseLoader color="#0b0a14ff" size={8} />
              </div>
            ) : comments.length !== 0 ? (
              comments.map((comment2, index) => {
                return (
                  <>
                    <div
                      className="flex flex-row gap-3 p-2 rounded-md  "
                      key={comment2._id}
                    >
                      ...
                      <div className="flex flex-col  flex-1 relative">
                        <div className="flex justify-between">
                          ...
                          {comment2.UserId._id === user._id && (
                            <p
                              className="absolute right-0 top-0  rounded-full  flex p-1 hover:bg-gray-300 transition ease-in cursor-pointer"
                              onClick={() => {
                                handlDeleteComment(comment2._id);
                              }}
                            >
              ...
              <div className="absolute top-50 right-27 font-semibold">
                No comments
              </div>
            )}
          </div>
        ...
        <div className="flex items-start justify-between mb-3    to-white p-2 rounded-xl">
          ...
          {/* Three-dot menu */}
          <ThreeDotMenu PostUserId={PostUserId} PostId={PostId}></ThreeDotMenu>
        </div>
        ...
        {/* Footer actions */}
        <div
          className="flex items-center gap-8 pt-3 border-t border-gray-100 text-gray-500 text-sm"
          disabled={disabled}
        >
          <button
            onClick={toggleLike}
          ...
          >
            <span>
              {liked ? (
                ...
                <img src={unlike} className="w-7 h-7"></img>
              )}
            </span>
            {likeCount}
          </button>
