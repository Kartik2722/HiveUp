...
export default function CreatePost() {
    ...
    const Fetchdata = async () => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                if (data.Post.image) {
                    // data.Post.image = `http://localhost:5000${data.Post.image}`;
    ...
    return (
        ...
        <>
            {getPostLoading ? <Spinner text="wait...." fullScreen={true} /> :
                <div className="relative min-h-[85vh]  bg-gradient-to-b from bg-pink-50 via-purple-100 to-indigo-50 px-6 py-16 flex flex-col items-center">
                    <div className="text-center mb-8">
                        <h1 className="text-2xl font-bold text-gray-900 mb-2"> {EditStatus ? "Update Your" : "Create"} Post</h1>
                                ...
                                <div className=" flex justify-start">
                                    ...
                                    {preview &&
                                        <div className="relative">
                                            <img src={preview} className=" rounded-lg w-20 h-20" alt="" />
                                            <button
                        ...
                        <div className="border-t border-gray-300 mt-4 pt-4 flex items-center justify-between">
                            ...
                            <div className="relative inline-flex items-center justify center rounded-xl">
                                <button
                                    ...
                                    onClick={submitPost}
                                    className={`bg-indigo-700 disabled:bg-indigo-500 disabled:cursor-not-allowed ${loading ? "disabled:bg-indigo-500 disabled:cursor-not-allowed" : ""} hover:bg-indigo-600 transition  font-semibold p-3 px-4 cursor-pointer rounded-xl   ${loading ? "text-transparent" : "text-white"} `}>{EditStatus ? "Update" : "Create"}  </button>
                                {
                                    loading &&
            ...
            {showLocationModal && (
                <LocationModal onClose={() => setShowLocationModal(false)} setLocation={setLocation} />
            )}
        </>
    )
}